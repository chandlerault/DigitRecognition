...
def fn_linear(input, params, hyper_params, backprop, dv_output=None):
    ...
    if backprop:
        ...
        dv_input = W.transpose() @ dv_output
        grad['W'] = dv_output @ input.transpose()/batch_size

        grad['b'] = dv_output.mean(axis=1)
        grad['b'] = np.reshape(grad['b'], params['b'].shape)
